# KEESS `/kium` B-Type 고도화 기술명세서 v1.1

- **작성일**: 2026-09-04 (v1.1 개정 동일자)
- **개정 이력**: v1.0 최초 · **v1.1** — 인트로 카피 사업 확정본 반영 / 정부지원 환급 **승격 1줄 삭제**(대체 문구 없이 제거로 확정)
...
[작업 2] 카운트·라벨 (§3) — P0
  ...
  섹션 헤더 필터 연동 / 인트로 카피 / 결과 문구 시각 노출.
  분야·모집 상태 칩은 무변경입니다.
  ※ 정부지원 환급은 카드 배지만 제거하고 **대체 문구를 신설하지 마세요**(§3-4).
    공개교육 보기의 '1명부터 신청 가능 · 정부지원 환급' 줄은 그대로 둡니다.
