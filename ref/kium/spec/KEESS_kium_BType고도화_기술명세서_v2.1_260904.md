# KEESS `/kium` B-Type 고도화 기술명세서 v2.1

- **작성일**: 2026-09-04 (v1.1 개정 동일자)
- **개정 이력**: v1.0 최초 · v1.1 인트로 카피 확정 · **v2.0** — 중복 노출 제거 6건 반영(§3-6~§3-10 · §5-2·§5-3) / **v1.1 §3-6 오류 정정** · **v2.1** — 회차 카드 요소 축소 2건(§5-4 과정명 밑줄 · §5-5 상태·CTA 통합)
...
| **BT-19** | 회차 카드 **과정명 밑줄 제거** + hover·focus 시 복원 | P1 |
| **BT-20** | 회차 카드 **상태 배지 + CTA를 단일 버튼으로 통합** (마감은 형태 교체) | P1 |
...
### 5-4. BT-19 회차 카드 과정명 밑줄 제거 — `styles/kium-open.css` (350~355행)

#### 현행

```css
.kium-scard2-course{ …; color:var(--p1); font-weight:800;
  text-decoration:underline; text-underline-offset:3px; }
.kium-scard2-course:hover,.kium-scard2-course:focus-visible{ color:var(--ink) }  /* 색만 바뀐다 */
.kium-scard2-course.is-static{ text-decoration:none; color:var(--ink) }
```

이 과정명은 버튼이며(`onCourseClick` → 그리드의 해당 과정 카드로 이동), 밑줄은 그 어포던스다. 다만 **스트립 6장 + 상세 패널 회차 카드**에 반복되어 시각 노이즈가 된다.

#### 판단

| 항목 | 내용 |
| --- | --- |
| 제거 후 남는 단서 | **색(`--p1`) + 굵기(800)** — 이웃 `.kium-scard2-days`(`--muted` 700)와 확연히 다르다 |
| 대비 검토 | `--p1`(#2E1A6B) ↔ `--muted`(#54585f) 대비 **약 1.7:1**. WCAG G183이 권하는 **3:1에 미달**하므로 색만으로는 부족하다 |
| **필수 조건** | **hover·focus-visible에서 밑줄을 복원**한다. 현행은 색만 바뀌므로 이 부분도 함께 고친다 |
| 터치 기기 | hover가 없어 색 단서만 남는다. 다만 이 링크는 **보조 경로**이고(주 행동은 CTA, 같은 과정 카드가 바로 아래 그리드에 있다) 발견 실패 비용이 낮다 |
| 접근성 | `aria-label="{과정명} 과정 카드로 이동"`이 이미 있어 **스크린리더 영향 0** |
| 뷰 일관성 | 「전체 일정」 리스트의 `.kium-srow-title`은 **정적 `<span>`**(밑줄 없음)이다. 밑줄을 빼면 **두 뷰의 시각이 통일**된다 |

#### 확정

```css
/* 수정 후 — 기본 상태 밑줄 없음. 어포던스는 hover·focus에서 준다 */
.kium-scard2-course{border:0;background:transparent;text-align:left;
  padding:12px 0;margin:-12px 0;
  font-size:13.5px;font-weight:800;color:var(--p1);line-height:1.45;word-break:keep-all;
  text-decoration:none;text-underline-offset:3px;
  transition:color .18s var(--ease)}
.kium-scard2-course:hover,.kium-scard2-course:focus-visible{
  color:var(--ink);text-decoration:underline}
.kium-scard2-course.is-static{text-decoration:none;color:var(--ink)}
```

`padding`/`margin`(44px 히트 영역) · `is-static` 규칙은 **무변경**.

### 5-5. BT-20 상태 배지 + CTA 통합 — `SessionBadge.tsx` · `SessionCard.tsx` ★

#### 검토한 제안과 판정

> **제안**: `이 일정으로 상담` 버튼을 없애고 상태 배지(`모집중`/`개강확정`/…) 자체를 버튼으로 쓴다. 마감은 클릭 이벤트를 걸지 않는다.

| 관점 | 판정 |
| --- | --- |
| 문제의식 | **정확하다.** 배지와 버튼이 세로로 붙어 같은 회차를 두 번 가리킨다. 카드 요소가 5개(날짜·일수·과정명·배지·버튼)로 많다 |
| 배지=버튼 그대로 | **권장하지 않는다.** ①`모집중`은 상태(명사)라 **버튼 라벨이 동작을 말하지 않는다** ②배지는 관례상 '정보'여서 클릭 가능하다고 학습돼 있지 않다 ③마감과 신청 가능 상태가 **같은 형태(배지)**라 무엇이 눌리는지 색에만 의존한다 |
| **채택안** | **배지를 버튼 안으로 흡수한다** — 요소는 2개→1개로 줄고, 라벨에 동작이 남는다 |

#### 확정 설계

```
┌────────────────────────────────┐
│ ⚪ 모집중            상담하기 → │   ← 전폭 버튼 1개 (기존 CTA가 이미 width:100%)
└────────────────────────────────┘
```

| 상태 | 렌더 | 클릭 |
| --- | --- | --- |
| `recruiting` | outline 버튼 · 좌 `⚪ 모집중` / 우 `상담하기 →` | 상담 폼(경로 A) |
| `confirmed` | outline 버튼 · 좌 `✓ 개강확정` / 우 `상담하기 →` | 상담 폼(경로 A) |
| `closing` | **filled 강조**(red 계열) · 좌 `⏰ 마감임박 (잔여 N석)` / 우 `마감 전 상담 →` | 상담 폼(경로 A) |
| `closed` | **버튼 아님** — `⊘ 마감` 정적 배지 + `다음 회차 상담 →` 텍스트 링크 | 링크만 동작(경로 B) |

> **마감은 형태 자체를 바꾼다.** `aria-disabled` 버튼은 스크린리더에 혼선을 주므로 요소를 교체한다 — 현행 `SessionCta`의 closed 처리 철학을 그대로 승계한다. "마감은 이벤트를 걸지 않는다"가 색이 아니라 **형태**로 표현된다.

#### 구현

`SessionBadge.tsx`에 **통합 컴포넌트를 신설**한다. 기존 `SessionBadge` · `SessionCta`는 `BadgeShowcase` · `SessionListView`가 계속 쓰므로 **삭제하지 않는다.**

```tsx
/** 회차 카드 전용 — 상태와 행동을 한 버튼에 담는다(§5-5). 리스트 행(.kium-srow)에는 쓰지 않는다 */
export function SessionAction({
  status, seatsLeft, label, onClick, onNext,
}: {
  ...
  if (status === 'closed') {
    return (
      <div className="kium-sact-closed">
        <span className="kium-sbadge" data-tone="gray"><Icon size={14} /><span>{meta.label}</span></span>
        <button type="button" className="kium-cta-next" onClick={onNext}
                aria-label={`${label} 다음 회차 상담`}>
          다음 회차 상담
        </button>
      </div>
    );
  ...
  return (
    <button
      ...
      data-tone={meta.tone}
      onClick={onClick}
      aria-label={`${label} ${meta.label} ${CTA_LABEL[status]}`}
    >
...
// 수정 후 — 카드에서만 통합 버튼을 쓴다
<SessionAction
  status={st}
  seatsLeft={session.seatsLeft}
  label={a11y}
  onClick={() => onConsult(session)}
  onNext={() => onConsult(session)}   // consultSession이 마감 시 경로 B로 넘긴다(기존 로직)
/>
```

| 적용 범위 | 내용 |
| --- | --- |
| 적용 | `.kium-scard2` — 스트립 카드 · 상세 패널 회차 카드(같은 컴포넌트) |
| **미적용** | `SessionListView`의 `.kium-srow` — **가로 배치**라 배지와 CTA가 이미 좌우로 분리돼 있고 세로 절약 효과가 없다. 현행 유지 |
| 효과 | 카드 요소 5개 → **4개**. 세로 1줄 절약 — 모바일 가로 스크롤 카드에서 특히 유효 |
...
/* ── 회차 카드 상태·CTA 통합 버튼 (BT-20) ── */
.kium-sact{display:flex;align-items:center;justify-content:space-between;gap:10px;
  width:100%;min-height:44px;margin-top:2px;padding:0 12px;
  border:1px solid var(--line);border-radius:10px;background:#fff;
  font-size:13px;font-weight:700;color:var(--ink);
  transition:border-color .18s var(--ease),background .18s var(--ease)}
.kium-sact:hover,.kium-sact:focus-visible{border-color:var(--p1);background:var(--surface)}
.kium-sact-st{display:inline-flex;align-items:center;gap:5px;flex:none}
.kium-sact-st em{font-style:normal;font-size:11.5px;font-weight:700;color:var(--muted);margin-left:2px}
.kium-sact-go{display:inline-flex;align-items:center;gap:4px;color:var(--p1);font-weight:800}
/* 마감임박만 지면에서 유일하게 filled — 긴박감이 곧 정보다 */
...
.kium-sact-closed{display:flex;align-items:center;justify-content:space-between;gap:10px;
  width:100%;margin-top:2px}
@media(max-width:479px){
  .kium-sact{padding:0 10px;font-size:12.5px}
}
...
- [ ] 회차 카드 과정명 **기본 상태 밑줄 없음** · hover·focus 시 밑줄 복원
- [ ] 회차 카드 상태·CTA가 **버튼 1개**로 통합 (2요소 → 1요소)
- [ ] 마감 회차는 **버튼이 아니라** 정적 배지 + 텍스트 링크
- [ ] `SessionListView`의 `.kium-srow`는 **현행 유지**(통합 미적용)
- [ ] `BadgeShowcase`가 기존 `SessionBadge`·`SessionCta`로 정상 동작
...
[작업 4-2] 일정 영역·CTA (§3-10 · §5-2 · §5-3) — P0
  ...
  과정명 밑줄 제거 + hover·focus 복원(§5-4).
  상태 배지 + CTA를 SessionAction 버튼 1개로 통합(§5-5).
    마감은 버튼이 아니라 정적 배지 + 텍스트 링크로 형태를 바꿉니다.
    기존 SessionBadge·SessionCta는 삭제하지 마세요(BadgeShowcase·SessionListView가 사용).
    SessionListView의 .kium-srow에는 통합을 적용하지 마세요.
...
[작업 7] 검증 후 표로 보고
  ...
  (6-3) 회차 카드 상태·CTA 통합 — 4상태 전건 렌더, 마감은 버튼 아님, 과정명 밑줄 없음
...
| D12 | 스트립 과정명의 **클릭 기능 자체를 없앨지** — 리스트 뷰에는 없는 기능이고 같은 과정 카드가 바로 아래 그리드에 있다 (§5-4) | 낮음 | 기능 유지 |
